...
void osrv::IOnvifService::Run()
{
	...
	http_server_->resource[search_xaddr]["POST"] = [this](std::shared_ptr<HttpServer::Response> response,
		std::shared_ptr<HttpServer::Request> request) {
			...
			//handleRequest(response, request->content);
	};

	is_running_ = true;
}

//void /*osrv::IOnvifService::*/handleRequest(std::shared_ptr<std::ostream> response,
//			std::istream& request)
//{
...
//	try
//	{
//		pt::xml_parser::read_xml(request, *tree);
//		auto* ptr = static_cast<exns::Parser*>(tree);
//		method = static_cast<exns::Parser*>(tree)->___getMethod();
//	}
//	catch (const pt::xml_parser_error& e)
...
//	}
//
//	auto handler_it = std::find_if(requestHandlers_.begin(), requestHandlers_.end(),
//		[&method](const HandlerSP handler)
//		{
//			return handler->get_name() == method;
//		}
//	);
//
//	if (handler_it != requestHandlers_.end())
//	{
//		//check user credentials
//		try
//		{
...
//			//extract user credentials
//			osrv::auth::USER_TYPE current_user = osrv::auth::USER_TYPE::ANON;
//			if (server_configs_->auth_scheme_ == AUTH_SCHEME::DIGEST)
//			{
//				auto auth_header_it = request->header.find(utility::http::HEADER_AUTHORIZATION);
//				if (auth_header_it != request->header.end())
//				{
...
//		{
//			std::ostringstream oss;
//			oss << "A server's error occured in " << service_name_  << " while processing: "
//				<< method <<  ". Info: " << e.what();
//			log_->Error(oss.str());
//
//			*response << "HTTP/1.1 500 Server error\r\nContent-Length: " << 0 << "\r\n\r\n";
...
//	{
//		std::ostringstream oss;
//		oss << "Not found an appropriate handler in " << service_name_ << " for: " << method;
//		log_->Error(oss.str());
//		*response << "HTTP/1.1 400 Bad request\r\nContent-Length: " << 0 << "\r\n\r\n";
//	}
//}
