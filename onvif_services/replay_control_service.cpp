...
namespace osrv
{
	struct GetReplayHandler : public OnvifRequestBase
	{
			...
			std::shared_ptr<HttpServer::Request> request) override
		{
			...
			const std::string uri = "rtsp://" + srv_configs_->ipv4_address_
				+ ":" + srv_configs_->rtsp_port_ + "/Recording0";


			auto envelope_tree = utility::soap::getEnvelopeTree(ns_);
			envelope_tree.add("s:Body.trp:GetReplayUriResponse.trp:Uri",
				uri);
	...
	ReplayControlService::ReplayControlService(const std::string& service_uri,
