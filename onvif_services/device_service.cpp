...
namespace osrv
{
	struct GetCapabilitiesHandler : public OnvifRequestBase
	{
		GetCapabilitiesHandler(const std::map<std::string, std::string>& xs, const std::shared_ptr<pt::ptree>& configs,
			...
			: OnvifRequestBase(GetCapabilities, auth::SECURITY_LEVELS::PRE_AUTH, xs, configs)
			, srv_cfgs_(server_cfg)
			...
			std::shared_ptr<HttpServer::Request> request) override
		{
			...
			utility::soap::jsonNodeToXml(capabilities_config, capabilities_node, "tt",
				XAddrProcessor(srv_addr_));
	...
	{
		GetServicesHandler(const std::map<std::string, std::string>& xs, const std::shared_ptr<pt::ptree>& configs, const std::string& ip)
	...
	DeviceService::DeviceService(const std::string& service_uri, const std::string& service_name, std::shared_ptr<IOnvifServer> srv)
		: IOnvifService(service_uri, service_name, srv)
	{
		requestHandlers_.push_back(std::make_shared<GetCapabilitiesHandler>(xml_namespaces_, configs_ptree_, *srv->ServerConfigs(), srv->ServerAddress()));
		...
		requestHandlers_.push_back(std::make_shared<GetServicesHandler>(xml_namespaces_, configs_ptree_, srv->ServerAddress()));
