...
IOnvifServer::IOnvifServer(const std::string& configs_path, std::shared_ptr<ILogger> logger)
		: configs_path_(configs_path), logger_(logger), http_server_(new osrv::HttpServer()),
			media_profiles_manager_(std::make_shared<utility::media::MediaProfilesManager>(
					ConfigPath(configs_path_, ConfigName(PROFILES_CONFIG_FILE))))
{
}
...
std::shared_ptr<IOnvifService> IOnvifServer::DeviceService()
{
	if (!device_service_)
	{
		device_service_ = OnvifServiceFactory().Create(SERVICE_URI::DEVICE, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::DeviceIOService()
{
	...
	{
		deviceio_service_ = OnvifServiceFactory().Create(SERVICE_URI::DEVICEIO, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::ImagingService()
{
	if (!imaging_service_)
	{
		imaging_service_ = OnvifServiceFactory().Create(SERVICE_URI::IMAGING, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::MediaService()
{
	if (!media_service_)
	{
		media_service_ = OnvifServiceFactory().Create(SERVICE_URI::MEDIA, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::Media2Service()
{
	if (!media2_service_)
	{
		media2_service_ = OnvifServiceFactory().Create(SERVICE_URI::MEDIA2, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::PTZService()
{
	if (!ptz_service_)
	{
		ptz_service_ = OnvifServiceFactory().Create(SERVICE_URI::PTZ, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::RecordingSearchService()
{
	...
	{
		recording_search_service_ = OnvifServiceFactory().Create(SERVICE_URI::SEARCH, shared_from_this());
	}
...
std::shared_ptr<IOnvifService> IOnvifServer::ReplayControlService()
{
	...
	{
		replay_control_service_ = OnvifServiceFactory().Create(SERVICE_URI::REPLAY, shared_from_this());
	}
