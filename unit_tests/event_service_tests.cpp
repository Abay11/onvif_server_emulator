...
{
	StringPairsList_t source_props = {{"InputToken", "tt:ReferenceToken"}, {"SomeConfig", "tt:Reference"}};
	StringPairsList_t data_props = {{"LogicalState", "xs:boolean"}};
	...
	BOOST_TEST("InputToken" == source_item_descriptions_tree.front().second.get<std::string>("<xmlattr>.Name"));
	BOOST_TEST("tt:ReferenceToken" == source_item_descriptions_tree.front().second.get<std::string>("<xmlattr>.Type"));
	BOOST_TEST("SomeConfig" == source_item_descriptions_tree.back().second.get<std::string>("<xmlattr>.Name"));

	BOOST_TEST(
			"LogicalState" ==
			res_tree.get<std::string>("DigitalInput.tt:MessageDescription.tt:Data.tt:SimpleItemDescription.<xmlattr>.Name"));
	BOOST_TEST("xs:boolean" == res_tree.get<std::string>(
																 "DigitalInput.tt:MessageDescription.tt:Data.tt:SimpleItemDescription.<xmlattr>.Type"));
}