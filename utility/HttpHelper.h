...
{
	UserCredentials() : type(osrv::auth::USER_TYPE::ANON)
	{
	}
...
inline void fillResponseWithHeaders(std::ostream& os, const std::string& content,
																		HeadersWriter* writer = NoErrorDefaultWriter)
{
	writer(os, content);
}
...
{
	RequestHandlerBase(const std::string& name, osrv::auth::SECURITY_LEVELS lvl) : name_(name), security_level_(lvl)
	{
	}
